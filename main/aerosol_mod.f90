...
PUBLIC :: diameter, particle_mass, particle_volume, particle_conc, &
...
SUBROUTINE nucleation(nucleation_coef,conc_h2so4,particle_conc,dt_aerosol) ! (Add input and output variables here)
  ...
  REAL(dp) :: conc_h2so4 ! [molecules/m^3] Concentration of sulphuric acid.
  ...
  ! in the first size bin particle_conc(1) within one model time step
  J_2nm = nucleation_coef * conc_h2so4**2
  particle_conc(1) = particle_conc(1) + J_2nm*dt_aerosol ! Update the 2 nm size bin
...
SUBROUTINE condensation(timestep, temperature, pressure, mass_accomm, molecular_mass, &
  ...
  do j = 1, nr_bins
    ...
    cond_sink(1) = cond_sink(1) + CR(1) * particle_conc(j)
    cond_sink(2) = cond_sink(2) + CR(2) * particle_conc(j)
...
REAL(dp), DIMENSION(25) :: vapour_conc
REAL(dp), DIMENSION(nr_bins), INTENT(IN) :: diameter
REAL(dp), INTENT(IN) :: timestep
...
!write(*,*) exp(-deposition_particle(i) * timestep / mixing_height)
end do
! Then the gasses
vapour_conc(20) = vapour_conc(20)*exp(-deposition_gas(1) * timestep / mixing_height) ! SO2
vapour_conc(1) = vapour_conc(1)*exp(-deposition_gas(2) * timestep / mixing_height) ! O3
vapour_conc(17) = vapour_conc(17)*exp(-deposition_gas(3) * timestep / mixing_height) ! HNO3
vapour_conc(13) = vapour_conc(13)*exp(-deposition_gas(4) * timestep / mixing_height) ! isoprene
vapour_conc(23) = vapour_conc(23)*exp(-deposition_gas(5) * timestep / mixing_height) ! a-pinene
